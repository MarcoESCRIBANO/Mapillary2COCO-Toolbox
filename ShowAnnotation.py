...
for i, ann in enumerate(anns):
    ...
    image_path = os.path.join(coco_dataset_path, "images", image_path)
    print(image_path)
    ...
    cv2.imwrite("coco/polygan_label.png", image)
    # cv2.waitKey()
...
for ann in anns:
    ...
    cv2.imwrite("coco/RLE_label1.png", label*255)
    # cv2.waitKey()
    ...
    cv2.imwrite("coco/RLE_label.png", image)
    
    background = cv2.imread(image_path)
    overlay = cv2.imread("coco/RLE_label1.png")

    added_image = cv2.addWeighted(background,1,overlay,0.5,0)
    cv2.imwrite("coco/Labelled_image.png", added_image)
    # cv2.waitKey()

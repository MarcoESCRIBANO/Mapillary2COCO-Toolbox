...
# IMAGE_NAME = "" means all images 
# It can be a list of images ex: IMAGE_NAME = "_1Gn_xkw7sa_i9GU4mkxxQ.jpg,E00oWA0zsmrvpDmZGxNw_w.jpg"
IMAGE_NAME = ""  
...
for ann in anns:
    ...
    if IMAGE_NAME == "" or image_path in IMAGE_NAME:
...
for i, ann in enumerate(anns):
    ...
    if IMAGE_NAME == "" or image_path in IMAGE_NAME:
        ...
        for seg in segs: image = cv2.drawContours(image, seg, -1, tuple(color), 3)

        cv2.rectangle(image, (int(bbox[0]), int(bbox[1])), 
                    (int(bbox[2]), int(bbox[3])), tuple(color), 3)
